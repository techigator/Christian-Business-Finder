<?php
$project_open = App\Models\attributes::where('attribute', 'project')
    ->where('is_active', 1)
    ->get();
?>
<div class="modal fade" id="exampleModaltooltip2" tabindex="-1" role="dialog" aria-labelledby="exampleModalLongTitle2"
                    ...
                    @if ($project_open)
                        @foreach ($project_open as $project)
                            <a href="{{ route('switch_project', [$project->id]) }}">
                                <div class="col-12 col-md-12 col-lg-12 mt-12 role-assign-modal"
                                    data-role_id="{{ $project->id }}" data-rolename='{{ $project->role }}'
                                            ...
                                            <div class="card-body p-4">
                                                <div class="d-flex">
                                                    <div class="circle-50 outline-badge-primary"
                                                        style="color: {{ $project->color }};border: 1px solid {{ $project->color }};">
